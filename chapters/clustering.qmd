...
In ST data, we can apply clustering algorithms to identify 'spatial domains', which represent spatially defined regions consisting of consistent gene expression profiles. In many (but not all) cases, spatial domains consist of regions with a single cell type or a consistent mixture of cell types.

If we apply clustering algorithms from single-cell workflows to ST data (without including any spatial information), then this is the same as applying clustering to identify cell types in single-cell data (although each spot may contain one or more cells, depending on the platform and tissue cell density).

It is important to keep in mind that cell types and states can be defined at various resolutions (or on a continuum). For the purposes of clustering, this means that the optimal number of clusters depends on the biological context. There is no "true" number of clusters -- this depends on the biological context (e.g. major cell populations vs. rare subtypes).

Once we have identified spatial domains by clustering or manual analyses, these can then be further investigated by differential expression testing to identify representative genes.
...
In ST data, we can also perform clustering that takes spatial information into account, for example to identify spatially compact or spatially connected clusters.
