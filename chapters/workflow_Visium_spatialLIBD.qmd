...
knitr::include_graphics("../images/spatialLIBD_Figure1.png")
```
...
Note that we have copied the default [`www` directory files from the `spatialLIBD` repository](https://github.com/LieberInstitute/spatialLIBD/tree/master/inst/app/www) and [adapted them to our liking]((https://github.com/lmweber/BestPracticesST/blob/main/spatialLIBD_demo/www)). We then use these files with `spatialLIBD::run_app(docs_path)` in our `app.R` script. These files help us control portions of our `spatialLIBD`-powered website and customize it to our liking.
