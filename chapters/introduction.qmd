...
In general, we focus on downstream analysis methods, which start with a gene count table and spatial coordinates as the main inputs. Preprocessing procedures to generate gene count tables are covered in other resources (e.g. our related book [Visium Data Preprocessing](https://lmweber.org/Visium-data-preprocessing/) for data from the 10x Genomics Visium platform) and tutorials provided by the manufacturers of each technological platform.

Many downstream analysis methods are generally applicable across technological platforms. For molecule-based platforms, we will usually use data that has been aggregated to the cell level.
...
All methods used in this book are available through Bioconductor or CRAN (in R) or PyPI (in Python) to ensure installability and long-term maintenance. In some chapters we also mention additional methods available as packages from GitHub, but these will not be included within the interactive code examples.
