...
```{r plot-qc-2d, fig.width=3, fig.height=3}
#| code-fold: true
df <- data.frame(colData(spe), spatialCoords(spe))
...
#| code-fold: true
df$foo <- df$total_counts/df$cell_area > 1
ggplot(df, aes(x_centroid, y_centroid, col=foo)) +
    geom_point(shape=19, stroke=0, size=0.1, show.legend=FALSE) +
...
To not confuse RNA target with negative probe counts, we will move these from the main data into an `altExp`, and compute basic cell-level QC metrics for both RNA target and negative probes:

```{r cosmx-loading, message=FALSE}
spe <- eh[["EH9525"]]
neg <- grep("NegPrb", rownames(spe))
altExp(spe, "NegPrb") <- SingleCellExperiment(list(counts=counts(spe)[neg, ]))
...
fd <- df |>
    group_by(fov) |>
    summarise(across(ends_with("global_px"), median))
ggplot(df, aes(CenterX_global_px, CenterY_global_px, col=factor(fov))) +
...
```{r comsmx-plotting, warning=FALSE, fig.width=10, fig.height=3}
#| code-fold: true
df <- data.frame(colData(spe), d) |>
    pivot_longer(colnames(d), values_to="d")
fd <- df |> 
    group_by(name) |> arrange(d) |> 
    mutate(across(c(d, sum), rollmean, k=100, align="right", fill=NA))
ggplot(df, aes(d, sum)) + facet_grid(~name) +
    geom_point(shape=16, size=0.4, alpha=0.2, col="navy") +
    geom_line(data=fd, col="blue", linewidth=0.4) +
