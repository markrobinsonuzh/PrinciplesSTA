...
Platforms differ drasticallly in terms of the experimental procedures used (sequence or ion counts versus fluorescence intensities), the number of features that can be identified (10s of proteins in imaging mass cytometry to full transcriptome in Visium), and spatial resolution (e.g., single-cell resolution or multiple cells per measurement location). In general, that also means there are tradeoffs between the number of features, spatial resolution, and sensitivity of the assays.
