# Workflow: Human DLPFC {#sec-human-dlpfc-workflow}
...
Next, calculate spot-level QC metrics using the `scater` package [@McCarthy2017], and store the QC metrics in `colData`. See @sec-quality-control for more details, including explanations of the QC metrics.
...
Calculate log-transformed normalized counts (logcounts) with the library size factors methodology, using methods from `scater` [@McCarthy2017] and `scran` [@Lun2016], making the assumption that spots can be treated as equivalent to cells. For more details, see @sec-normalization.
...
Identify a set of top highly variable genes (HVGs), which will be used to define cell types. We use methods from `scran` [@Lun2016], treating spots as equivalent to single cells, and considering only molecular features (gene expression) as described in @sec-feature-selection. We also first filter out mitochondrial genes, since these are very highly expressed and not of main biological interest here.
...
Next, we perform clustering to define cell types. Here, we use molecular features (gene expression) only, as described in @sec-clustering. We apply graph-based clustering using the Walktrap method implemented in `scran` [@Lun2016], applied to the top 50 PCs calculated on the set of top HVGs.
